...
LRESULT CALLBACK OGLApplication::WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	switch(msg) {
		...
		case WM_MBUTTONUP:
			s_oglapp->getWindow()->processMouse(MIDDLE, RELEASED, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
		break;

		case WM_MBUTTONDOWN:
			s_oglapp->getWindow()->processMouse(MIDDLE, PRESSED, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
		break;

		case WM_MOUSEWHEEL:
			s_oglapp->getWindow()->processMouseWheel(GET_WHEEL_DELTA_WPARAM(wparam) < 0 ? WHEEL_UP : WHEEL_DOWN, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
		break;
